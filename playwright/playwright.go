...
import (
	...
	"github.com/MarcosIgnacioo/classmoodls/classroomapi"
	"github.com/MarcosIgnacioo/classmoodls/helpers/arraylist"
	"github.com/MarcosIgnacioo/classmoodls/types"
...
func StartScrapping(user types.User) {
	...
	ClassroomAPIAuth(&browser, *authURL, user.Username, user.Password)
	// ms, logErr := MoodleScrap(&browser, user.Username, user.Password)
}

func FuckAround(username string, password string, APIKey *string) (*ScrappedInfo, *LoginError) {
	// TODO: Crear un package con variables globales (Expect)
